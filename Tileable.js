...
], function(declare, win, connect, domClass, domConstruct, geometry, style, WidgetBase, ScrollableMixin){
...
=====*/
	...
	return declare("Tileable", [WidgetBase, ScrollableMixin], {
		// summary:
		//		The base class for tiles
		...
		// left style attribute of the left top tile
		_left: 0,
		_top: 0,
		
		constructor: function(kwArgs) {
			if (!dojo.isArray(this.tileSize)) this.tileSize = [this.tileSize, this.tileSize];
			if (!kwArgs.center) this.center = [0,0];
			if (!kwArgs.getTileContent) this.getTileContent = function(){};
			this.tiles = [];
			this.tileBounds = [];
		...
		startup: function(){
			...
			connect.connect(this.touchNode, "ondblclick", this, function(event){
		...
		updateTileDivs: function() {
			...
			for (var i1=0; i1<this.numTilesX; i1++) {
				...
				for (var i2=0; i2<this.numTilesY; i2++) {
					...
					if ( inVerBounds && (inHorBounds || this.wrapHor) ) {
						...
						this.getTileContent(tile, this.zoom, x, y);
					}
					else if (!tile.empty) {
						tile.div.innerHTML = "";
						tile.empty = true;
			...
			this.scrollTo({x: this.offsetX, y: this.offsetY}, 0);
		},
